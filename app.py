...
def resolve_db_uri():
    ...
    if url:
        # Render/Heroku forniscono "postgres://", SQLAlchemy con psycopg3 vuole "postgresql+psycopg://"
...
def calendar():
    ...
    if qdate:
        try:
            base = datetime.strptime(qdate, "%d-%m-%Y").date()
        except ValueError:
            base = date.today()
    else:
        base = date.today()
    ...
    return render_template("calendar.html", days=days, grouped=grouped, monday=monday,
                           prev_week=prev_week, next_week=next_week, format_date=format_date)
...
def clients_detail(client_id):
    ...
    return render_template("client_detail.html", client=client, appts=appts, format_date=format_date)
